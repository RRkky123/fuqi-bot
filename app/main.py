...
from app.routes import webhook, liff
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://liff.line.me", settings.base_url],
...
# 路由
app.include_router(webhook.router, prefix="", tags=["LINE Webhook"])
app.include_router(liff.router, prefix="", tags=["LIFF 購買"])
        ...
        async with AsyncSessionLocal() as db:
            for t in themes_data:
                result = await db.execute(
                    select(Theme).where(Theme.theme_id == t["theme_id"])
                )
                if not result.scalar_one_or_none():
                    db.add(Theme(**t))
            await db.commit()
    except Exception as e:
        logger.warning(f"主題種子失敗（可能 DB 尚未就緒）: {e}")
